...
const char * const BITCOIN_CONF_FILENAME = "gregcoin.conf";
...
fs::path GetDefaultDataDir()
{
    ...
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Gregcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Gregcoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Gregcoin";
#else
    // Unix-like
    return pathRet / ".gregcoin";
#endif
#endif
}
